# 처음 설정하기 — v7 날짜 진단·대회 통합·관심 공고

기존 설치자는 `UPGRADE.md`를 먼저 확인하세요. 신규 설치 설정은 아래와 같습니다.

# 처음 설정하기 — v5 제목·접수 일시·AI/데이터 관련성 보강

## v5의 기본 수집 정책

인하대 대표 홈페이지의 `인하대학교 · 교내 공지`는 자동 수집에서 제외하고 직접 확인 링크만 유지합니다. 다른 학과/센터 게시판은 계속 수집합니다. 기본값 `strict_ai_data: true`로 공고 자체의 AI·데이터 근거가 있는 항목만 화면·알림에 표시합니다. 제목이 단순 ‘공지사항’이면 저장된 목록 제목/게시글 제목으로 복구합니다. 마감 시각은 원문에서 확인한 경우만 표시합니다. 기존 사용자에게는 이 정책이 런타임 기본값으로 적용되므로 config를 전체 덮어쓸 필요가 없습니다. 자세한 변경·한계는 [UPGRADE.md](UPGRADE.md)에 있습니다.
...
이미 이전 버전을 사용 중이면 먼저 [UPGRADE.md](UPGRADE.md)를 읽으세요. **기존 `data/state.json`을 빈 파일로 덮어쓰면 안 됩니다.**
...
- `max_detail_requests: 30`: 상세 페이지 읽기 예산을 출처별로 번갈아 배분합니다. 일부 사이트가 전체 예산을 차지하지 않도록 했습니다. 한 번에 30개까지이며, v7 추출기로 아직 확인하지 않은 공고는 재확인 대상으로 포함하고, 접수 날짜가 불완전한 공고는 다음 날 재확인하며, 날짜가 확인된 공고는 기본적으로 3일 이후 다시 읽습니다. 새 공고가 많으면 모든 상세정보가 첫날 채워지지 않습니다. 목록 밖으로 밀려난 최근 공고와 최근 7일 이내 마감 공고도 예산 안에서 재확인합니다.
- `browser_fallback: true`: HTML에 링크가 없으면 공개 페이지를 Chromium으로 렌더링합니다. 로그인·CAPTCHA·403 차단을 우회하지 않습니다.
- `mode: contest` (아래 설명은 `strict_ai_data: false`일 때의 기본 모드이며, v5의 엄격한 관련성 필터가 켜져 있으면 공고 자체의 근거 확인이 추가됩니다): 대회 관련 표현을 기준으로 선별합니다. `ai_contest`는 AI/데이터 표현도 필요합니다. `platform`은 AI 대회 전용 플랫폼용입니다. 캠퍼스픽의 `ai_platform`은 공개 공모전 제목·카드의 AI/데이터/통계 등 키워드를 확인합니다. `AI영상`처럼 붙여 쓴 표현도 지원합니다. AI 영상·디자인 공모전도 포함되며, 관련 키워드가 본문에만 있는 공고는 놓칠 수 있습니다.
...
접수기간과 실제 대회기간은 서로 구분합니다. 접수 날짜는 ‘접수기간/신청기간/모집기간’ 등의 명시적 문맥과 연도 포함 날짜가 있어야 합니다. 같은 연도가 분명한 `2026.09.01 ~ 09.30`은 지원하되, 현재 연도를 임의로 대입하지 않습니다. 연도가 생략된 날짜는 공고 제목에 연도가 하나로 명시되었을 때만 그 연도를 적용하고, 상세 화면에 그 근거를 표시합니다. 원문에 적힌 `18:00` 같은 시각이 확인되면 정확한 마감·시작 상태 판정에 반영합니다. D-day 숫자는 한국 날짜 차이이며, 날짜만 확인되면 마감 시각을 추측하지 않습니다. 여러 트랙·차수의 접수기간은 하나로 합치지 않으며 **미확인**으로 표시합니다.
...
제작 환경에서는 외부 DNS 요청이 실패해 Python 수집기의 실제 사이트 접속, GitHub에서의 워크플로 실행과 Pages 배포, 휴대폰 수신을 종단간 검증하지 못했습니다. 공개 웹 도구로 캠퍼스픽 목록과 게시글 URL 형식을 확인했지만, 상세 페이지 본문은 읽기 결과에 노출되지 않았습니다. 합성 HTML 테스트 통과를 실제 캠퍼스픽 수집 성공으로 해석하면 안 됩니다. **예시 화면의 가상 데이터는 실사용 상태 파일에 넣지 않았습니다.**
...
첫 실행/첫 v3 실행은 기존 목록을 비교 기준으로만 저장하고, 다음 날부터 **어제 마지막 저장 목록 대비 새로 확인한 공고 주소 수**를 표시합니다. 같은 날 수동 갱신은 어제의 기준을 지우지 않습니다. v7은 여기에 근거 기반으로 통합한 대회·프로그램 수를 추가하고 새 공고 주소 수·재게시 수와 구분합니다. 실제 발표일을 확정하는 지표는 아닙니다. 전날 누락/수집 실패/새 출처 적재는 별도로 표시합니다.
...
첫 목록은 **마감 여유순**입니다. 확인된 미래 마감일의 내림차순 → 미확인 → 이미 마감 순서이며 기본 상태 필터는 마감 제외입니다. 마감 임박순과 최신순도 선택할 수 있습니다. 공고 제목은 카드에서 최대 두 줄, 상세정보에서 전체 제목을 봅니다.

날짜는 표/본문/설명 메타데이터, 같은 공고의 명시적인 일정 링크를 확인합니다. 데이콘의 개요 URL과 일정 탭을 구분해 요청합니다. 일부 정보만 있고 날짜가 빠졌어도 제한된 브라우저 보완을 시도합니다. 날짜 근거 문구·주소와 출처별 확인 건수를 표시합니다. JSON-LD의 Event 시작/종료는 대회기간이지 접수기간이 아니므로 섞지 않습니다.

기존 config.json을 바꾸지 않아도 추가 일정 페이지는 실행당 최대 18회, 상세 페이지 브라우저 보완 최대 8회, 상세 확인 단계 약 360초의 소프트 예산으로 제한됩니다. 개별 요청이 끝날 때 시간을 확인하므로 마지막 요청만큼 더 걸릴 수 있습니다. 원문 30개 제한은 기존 설정을 존중합니다. 포스터·PDF/HWP 안의 날짜는 이번 코드에서 읽지 않으며, 실제 접속 제한이 있으면 날짜가 여전히 미확인일 수 있습니다. 공고가 많으면 한 번에 모두 채워지지 않고 다음 수집 때 이어 확인합니다.

## v7 기능 확인과 진단 실행

화면 하단의 v7과 실행 상태에서 코드 지문·실제 실행 커밋·시각을 확인하세요. 제목 두 줄과 기본 마감 여유순은 그대로이며, 분야 필터·관심 저장·설정 내보내기가 추가되었습니다. 관심은 현재 브라우저에만 저장하고 무료 클라우드 알림에는 자동 반영하지 않습니다.

추가된 **Actions → Audit date collection (no notification)**는 실제 GitHub 환경에서 제한된 상세를 읽는 진단 전용 작업입니다. 상태 저장과 ntfy 전송 없이 Summary와 `date-quality-audit` artifact를 남깁니다. 처음 설치할 때 `.github/workflows/diagnose.yml`도 업로드하세요.

기존 사용자에게는 [UPGRADE.md](UPGRADE.md)가 우선입니다. 기능의 정확한 의미는 [FEATURES_V7.md](docs/FEATURES_V7.md), 수동 날짜 보정은 [DATE_CORRECTIONS.md](docs/DATE_CORRECTIONS.md), 실제 검증 범위는 [TEST_REPORT.md](docs/TEST_REPORT.md)를 참고하세요.
