...
from .operations import runtime_metadata
from .diagnostics import diagnostic_report,report_markdown
from .timing import as_local, timing_label,exact_instant,valid_day
from .taxonomy import technical_fields,LABELS as FIELD_LABELS

PUBLIC_FIELDS = ('id','title','url','source_id','source_name','group','posted_at','deadline','registration_start','platform_status','first_seen','last_seen','last_changed') + DETAIL_FIELDS
EVENT_FIELDS = ('notice_id','member_ids','favorite_ids','source_names','groups','source_count','grouping_basis','sources','cross_source_conflict','conflicting_deadlines')
...
def public_data(state:dict,now:datetime,demo:bool=False)->dict:
    review_items=[{k:v for k,v in row.items() if k in PUBLIC_FIELDS} for row in state.get('items',{}).values() if row.get('relevance_status')=='pending' and not row.get('duplicate_of')]
    groups=group_events(state)
    event_result=event_comparison(state,compare_day(state,now),groups)
    groups=[{k:v for k,v in e.items() if k in PUBLIC_FIELDS + EVENT_FIELDS} for e in groups]
    state=visible_state(state)
    threshold=(now-timedelta(hours=24)).isoformat()
    comparison=event_result
    ...
    for original in state['items'].values():
        item={k:v for k,v in original.items() if k in PUBLIC_FIELDS}
        for field in ('deadline','registration_start'):
            instant=exact_instant(item,field);item[field+'_at']=instant.isoformat() if instant else None
        item['title']=preferred_title(item) or '제목 확인 필요'
        item['status']=status_of(item,now);item['status_label']=timing_label(item,now);item['recent_new']=item['id'] in recent
    ...
    for event in groups:
        ...
        event['status']=status_of(event,now);event['status_label']=timing_label(event,now)
        event['daily_new']=event['id'] in comparison.get('new_event_ids',[])
        event['recent_new']=any(rid in recent for rid in event['member_ids'])
...
def deadline_sort_key(item, today):
    ...
    day=valid_day(item.get('deadline'))
    if status_of(item,today)=='closed':return (2,0,0,0,clean_title(item.get('title','')))
    if not day or item.get('registration_ambiguous'):return (1,0,0,0,clean_title(item.get('title','')))
    instant=exact_instant(item,'deadline')
    return (0,-day.toordinal(),0 if instant else 1,-instant.timestamp() if instant else 0,clean_title(item.get('title','')))

def md_text(text:str)->str:
    return html.escape(str(text)).replace('|','\\|').replace('\n',' ').replace('[','\\[').replace(']','\\]').replace('`','\\`')

def markdown_table(data:dict)->str:
    lines=['### 어제 대비 확인 결과', '', *[md_text(x) + '  ' for x in comparison_lines(data['daily_comparison'])], '', '공고 주소 수와 근거 기반으로 묶은 대회·프로그램 수를 구분합니다. 서로 다른 부문·차수는 별도입니다.', '', '| 출처 | 공고 / 사이트 | 접수기간 | 대회·행사기간 | 주최 / 참가 대상 | 상태 |','|---|---|---|---|---|---|']
    rows=[i for i in data.get('events',data['items']) if i['status']!='closed'][:100]
    for i in rows:
        url=canonical(i['url'])
        ...
        lines.append(f"| {md_text(i['source_name'])} | {cell} | {md_text(reg)} | {md_text(event)} | {md_text(info)} | {md_text(i.get('status_label') or LABEL[i['status']])} |")
...
def build(root:Path,state:dict,now:datetime,repo_url:str='',page_url:str='',demo:bool=False)->None:
    ...
    (site/'quality.json').write_text(json.dumps(public['quality'],ensure_ascii=False,indent=2)+'\n',encoding='utf-8')
    (site/'quality.md').write_text(report_markdown(public['quality']),encoding='utf-8')
    (site/'.nojekyll').touch()
    ...
    for i in public.get('events',public['items']):
        writer.writerow([csv_cell(v) for v in (i['title'],i['source_name'],i.get('registration_start'),i.get('deadline'),i.get('registration_text'),i.get('event_start'),i.get('event_end'),i.get('schedule_text'),i.get('organizer'),i.get('eligibility'),i.get('benefits'),i.get('summary'),i.get('website_url'),i.get('application_url'),i.get('status_label') or LABEL[i['status']],i['url'],i.get('detail_checked_at'),i.get('registration_start_time'),i.get('deadline_time'))])
