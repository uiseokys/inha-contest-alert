"""Non-destructive source and subject policy.
...
NOT_OPPORTUNITY=re.compile(r'참여자\s*선발\s*결과|참가자\s*선정\s*결과|선발\s*결과|합격자\s*발표|시험\s*결과\s*제출|장학금\s*신청|수료(?:자|증).*안내|수상\s*(?:소식|결과|실적)|수상자\s*발표|수상팀\s*발표|입상\s*소식|수강신청|정규\s*수업|대학원.*신입생|입학\s*전형|채용\s*공고|교수\s*초빙',re.I)
...
def opportunity_kind(title:str,url:str='')->str:
    parsed=urlsplit(url);host=parsed.hostname or ''
    if (host in ('dacon.io','www.dacon.io','aifactory.space','www.aifactory.space') and '/competitions/' in parsed.path) or (host in ('campuspick.com','www.campuspick.com') and parsed.path.startswith('/contest/view')):
        return 'contest'
    if re.search(r'공모|경진|해커톤|데이터톤|챌린지|콘테스트|대회|competition|contest|hackathon|challenge',title,re.I):return 'contest'
    if re.search(r'프로그램|부트캠프|교육|세미나|워크숍|특강|인턴십|캠프',title,re.I):return 'program'
    return 'other'
...
def classify(item:dict)->dict:
    ...
    if not evidence and '_topic_text' not in item and item.get('relevance_version') in (5,VERSION) and item.get('relevance_status')=='included':
        evidence=item.get('relevance_evidence') or ''
    if evidence:return dict(opportunity_kind=opportunity_kind(title,item.get('url','')),relevance_status='included',relevance_reason='공고 자체의 AI·데이터 주제 확인',relevance_evidence=evidence,relevance_version=VERSION)
...
def apply_policy(state:dict,config:dict)->None:
    from .repair import reconcile_identities,sanitize_fields
    reconcile_identities(state)
...
def visible_state(state:dict)->dict:
    ...
    items={rid:item for rid,item in state.get('items',{}).items() if item.get('relevance_status')=='included' and not item.get('duplicate_of')}
